...
namespace BackComp
{
    public partial class Form1 : Form
    {
        ...
        private void scan(String file)
        {
            ...
            List<double> points = getpoints(imhei, MAXHEI);
            for (double i = 0; i < imwid; i += imwid / MAXWID)
            {
                ...
                try
                {
                    for (double j = imhei - 1; j > 0; j -= points[points.Count - 1 - point])
                    {
                        if ((int)i < imwid && (int)j < imhei)
                        {
                            Color pixelColor = mypic.GetPixel((int)i, (int)j);
        ...
        private List<double> getpoints(int picsize, double Max)
        {
            double incr = picsize / Max;

            List<double> points = new List<double>();
            for (double i = 0; i < picsize; i += picsize / Max)
            {
                points.Add(i);
            }
            return points;
