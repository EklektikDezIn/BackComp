...
namespace BackComp
{
    public partial class Form1 : Form
    {
        ...
        private void scan(String file)
        {
            ...
            double[] points = getpoints(imhei, MAXHEI);
            for (double i = 0; i < imwid; i += imwid / MAXWID)
            {
                y = 0;
                int point = 0;
                for (double j = 0; j < imhei; j += points[point])
                {
                ...
                try
                {
                    for (double j = imhei - 1; j > 0; j -= points[points.Length-1 - point])
                    {
                        Color pixelColor = pixelColor = mypic.GetPixel((int)i, (int)j);
                        int value = (pixelColor.R + pixelColor.G + pixelColor.B) / 3;
                        if (x < MAXWID && y < MAXHEI)
                        {
                            BW[x, y] = (value < trackBar1.Value);
        ...
        private double[] getpoints(int picsize, double Max)
        {
            double incr = picsize / Max;

            double[] points = new double[(int)(Max/incr)];
            double start = 0;
